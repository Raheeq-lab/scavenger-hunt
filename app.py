...
def teacher_forgot_password():
    if request.method == 'POST':
        ...
        if teacher and teacher.school.lower().strip() == school.lower().strip():
            # In a real app, you'd send an email here. 
            # For this simple setup, we'll redirect to a reset page if security info matches.
            return render_template('reset_password.html', teacher_id=teacher.id)
        else:
            flash('Email and School name do not match our records.', 'danger')
